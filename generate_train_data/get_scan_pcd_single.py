...
class ScanPcdGenerator:
    def __init__(self, specs, is_visualize, mesh, logger):
        self.specs = specs
        self.logger = logger
        self.is_visualize = is_visualize
        self.mesh = mesh
        self.mesh_triangels = np.asarray(mesh.triangles)
    ...
    def get_points_intersect(self, projection_points: np.ndarray, cast_result):
        ...
        for ray_id, intersect_id in enumerate(geometry_ids):
            if intersect_id == 0:
                points_intersect.append(projection_points[ray_id])
        points_intersect = np.array(points_intersect).reshape(-1, 3)
        return points_intersect

    def is_view_legal(self, points_initial):
        """
        判定当前射线求交结果是否能够满足采集数据的要求
        Args:
            points_initial: 与obj相交的射线投影点
        Returns:
            是否满足采集条件
        """
        if points_initial.shape[0] == 0:
            return False
        return True
    ...
    def get_cur_view_pcd(self, cast_result):
        ...
        for i in range(hit.shape[0]):
            if not math.isinf(hit[i]):
                if geometry_ids[i] == 0:
                    points_pcd.append(
                        self.get_real_coordinate(self.mesh_vertices, self.mesh_triangels[primitive_ids[i]],
                                                 primitive_uvs[i]))

        pcd_scan = o3d.geometry.PointCloud()
        pcd_scan.points = o3d.utility.Vector3dVector(points_pcd)
    ...
    def get_current_view_scan_pcd(self, theta, phi):
        ...
        cast_result = self.get_ray_cast_result(self.scene, rays)  # 射线求交
        points_intersect = self.get_points_intersect(projection_points, cast_result)  # 与物体相交的射线投影点

        while points_intersect.shape[0] < pcd_sample_num:
            # 与物体相交的光线数量不够，则将原有光线在投影平面上进行扩充
            self.logger.info("intersect points not enough, cur: {}, target: {}"
                             .format(points_intersect.shape[0], pcd_sample_num))

            points_intersect = self.expand_points_in_rectangle(expand_points_num,
                                                               self.pixel_width,
                                                               self.pixel_height,
                                                               self.scan_plane,
                                                               points_intersect)
            rays = self.get_rays_from_projection_points(eye, points_intersect)
            cast_result = self.get_ray_cast_result(self.scene, rays)
            points_intersect = self.get_points_intersect(points_intersect, cast_result)

        # 多采集一些点，然后用fps保证均匀性
        pcd = self.get_cur_view_pcd(cast_result)
        pcd = pcd.farthest_point_down_sample(pcd_point_num)
    ...
    def generate_scan_pcd(self):
        ...
        pcd_scan, success = self.get_current_view_scan_pcd(0, 0)
        if success:
            pcd_partial_list.append(pcd_scan)
            scan_view_list.append(index)
            index += 1
        # 仰视
        self.logger.info("\n")
        self.logger.info("begin generate theta: {}, phi: {}".format(180, 0))
        pcd_scan, success = self.get_current_view_scan_pcd(180, 0)
        if success:
            pcd_partial_list.append(pcd_scan)
            scan_view_list.append(index)
            index += 1
        for theta in [45, 90, 135]:
            for phi in range(0, 360, 45):
                ...
                pcd_scan, success = self.get_current_view_scan_pcd(theta, phi)
                if success:
                    pcd_partial_list.append(pcd_scan)
                    scan_view_list.append(index)
                    index += 1

        return pcd_partial_list, scan_view_list
...
class TrainDataGenerator:
    def __init__(self, specs, logger):
        ...
        self.pcd_partial_list = []
        # the view index of scan point cloud
        self.scan_view_list = []

    def get_mesh(self):
        self.mesh = o3d.io.read_triangle_mesh(self.geometries_path["mesh{}".format(specs.get("obj_index"))])

    def get_init_geometries(self):
        self.get_mesh()

    def get_scan_pcd(self):
        scanPcdGenerator = ScanPcdGenerator(self.specs, self.specs["visualize"], self.mesh, self.logger)
        return scanPcdGenerator.generate_scan_pcd()

    def visualize_result(self, pcd_partial_list, scan_view_list):
        coor = geometry_utils.get_coordinate()
        sphere = geometry_utils.get_sphere_pcd()

        for i in range(len(pcd_partial_list)):
            o3d.visualization.draw_geometries(
                [pcd_partial_list[i], coor, sphere],
                window_name="{}".format(scan_view_list[i]))

    def save_pcd(self, specs, pcd_list, view_index_list, scene):
        """
        保存点云数据
        Args:
            specs: 配置信息
            pcd_list: 各个角度的partial point cloud
            view_index_list: 每个视角的index
            scene: 当前场景名
        """
        assert len(pcd_list) == len(view_index_list)
        ...
        for i in range(len(pcd_list)):
            # 获取点云名
            pcd_filename = '{}_view{}_{}.ply'.format(scene, view_index_list[i], int(specs.get("obj_index"))-1)

            # 保存点云
            pcd_path = os.path.join(pcd_dir, category, pcd_filename)
            if os.path.isfile(pcd_path):
                os.remove(pcd_path)
            o3d.io.write_point_cloud(pcd_path, pcd_list[i])

    def handle_scene(self, scene):
        self.geometries_path = path_utils.get_geometries_path(self.specs, scene)
        self.get_init_geometries()

        pcd_partial_list, scan_view_list = self.get_scan_pcd()

        if self.specs["visualize"]:
            self.visualize_result(pcd_partial_list, scan_view_list)

        self.save_pcd(self.specs, pcd_partial_list, scan_view_list, scene)
...
if __name__ == '__main__':
    config_filepath = 'configs/get_scan_pcd_single.json'
