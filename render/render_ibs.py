...
def create_materials():
    ...
    params = [
        ...
        {'name': 'mesh', 'color': (0.794, 0.4, 0.8, 1.0), 'transparent': False}
    ]
...
def launch_render(pcd_path: str, ibs_path: str, img_path: str, filename: str, additional_info: str):
    ...
    camera_obj: Object = bpy.data.objects['Camera']
    camera_location = np.array([0.7359, -0.6926, 0.8]) * 1.4
    ...
    ibs_filename = "{}.obj".format(ibs_basename)
    ply_file1 = pcd_path / ply_filename1
    ply_file2 = pcd_path / ply_filename2
    ibs_file = ibs_path / ibs_filename
    ...
    mesh1 = bpy.data.objects[ibs_basename]
    mesh1.active_material = bpy.data.materials['mesh']
...
if __name__ == '__main__':
    ...
    filename = 'scene6.1005'

    category = re.match("scene\\d", filename).group()
    scene_name = re.match("scene\\d.\\d{4}", filename).group()

    pcd_path = "D:\\dataset\\IBPCDC\\pcdComplete\\INTE\\{}".format(category)
    ibs_path = "D:\\dataset\\IBPCDC\\IBSMesh\\{}".format(category)
    img_path = "D:\\dataset\\IBPCDC\\render\\IBS\\{}\\{}".format(scene_name, filename)
    launch_render(pcd_path, ibs_path, img_path, scene_name, "ibs")
