...
from networks.model_Transformer_TopNet_INTE import *
...
def train(network, train_dataloader, lr_schedule, optimizer, epoch, specs, tensorboard_writer):
    ...
    for center, radius, pcd_partial, pcd_gt, idx in train_dataloader:
        ...
        train_total_loss_ibs += loss_ibs.item()
    ...
    train_avrg_loss_ibs = train_total_loss_ibs / train_dataloader.__len__()
    tensorboard_writer.add_scalar("train_loss_ibs", train_avrg_loss_ibs, epoch)
    logger.info('train_avrg_loss_ibs: {}'.format(train_avrg_loss_ibs))
...
def test(network, test_dataloader, epoch, specs, tensorboard_writer, best_cd_l1, best_epoch):
    ...
    with torch.no_grad():
        ...
        for center, radius, pcd_partial, pcd_gt, idx in test_dataloader:
            ...
            test_total_loss_ibs += loss_ibs.item()
            test_total_loss += loss_total.item()
        ...
        test_avrg_loss_ibs = test_total_loss_ibs / test_dataloader.__len__()
        tensorboard_writer.add_scalar("test_loss_ibs", test_avrg_loss_ibs, epoch)
        logger.info('test_avrg_loss_ibs: {}'.format(test_avrg_loss_ibs))
