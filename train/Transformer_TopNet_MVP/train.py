...
def test(network, test_dataloader, epoch, specs, tensorboard_writer, best_cd_l1, best_epoch):
    ...
    with torch.no_grad():
        ...
        for pcd_partial, pcd_gt, idx in test_dataloader:
            ...
            pcd_pred = network(pcd_partial)
            loss_cd_l1 = cd_loss_L1(pcd_pred, pcd_gt)
            loss_emd = emd_loss(pcd_pred, pcd_gt)
        ...
        test_avrg_loss_emd = test_total_emd / test_dataloader.__len__()
...
def main_function(specs):
    ...
    logger.info("begin time: {}".format(TIMESTAMP))
    ...
    TIMESTAMP = "{0:%Y-%m-%d_%H-%M-%S/}".format(datetime.now() + timedelta(hours=8))
    logger.info("end time: {}".format(TIMESTAMP))
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        ...
        default="configs/specs/specs_train_Transformer_TopNet_MVP.json",
        required=False,
    ...
    logger.info("begin time: {}".format(args.experiment_config_file))
