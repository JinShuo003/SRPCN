...
class IBS:
    ...
    def create_mesh(self):
        ...
        if self.clip == 'sphere':
            centroid1, scale1 = geometry_utils.get_pcd_normalize_para(self.points0)
            centroid2, scale2 = geometry_utils.get_pcd_normalize_para(self.points1)
            bound_mesh = None
            clip_scale = 1.5
            if scale1 < scale2:
                bound_mesh = pv.Sphere(scale1*clip_scale, centroid1)
            else:
                bound_mesh = pv.Sphere(scale2*clip_scale, centroid2)
            # pl = pv.Plotter()
            # pl.add_mesh(self.points0, "g")
            # pl.add_mesh(self.points1, "b")
            # pl.add_mesh(bound_mesh, "r")
            # pl.show()
